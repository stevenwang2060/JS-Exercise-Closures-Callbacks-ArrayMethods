...
function processLastItem(stringList, callback) {
  for (let i = 0; i < stringList.length; i++) {
    let newStr = callback(stringList.pop());
    return newStr;
  }
}
...
function processSum(numberList, callback) {
  let result = numberList.reduce((acc, curr) => (acc += curr), 0);
...
function processDuplicateFree(list, callback) {
  let result = list.filter((item, index) => list.indexOf(item) === index)
  return result
}
...
function getFullNames(runners) {
  let runnerArr = [];
  runners.forEach((runners) => runnerArr.push (`${runners.last_name}, ${runners.first_name}`))
  return runnerArr
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let result = runners.filter(runners => runners.shirt_size === tShirtSize)
  return result
}
...
function tallyUpDonations(runners) {
  let donate = runners.map(runners => runners.donation)
  let result = donate.reduce((acc, curr) => acc + curr, 0)
  return result 
}
 ...
 * etc
*/
const counterMakerWithLimit = (value) => {
  let count = 0;
  let maxValue = value
